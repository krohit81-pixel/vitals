...
export function RangeSelector({ range }: { range: RangeOption }) {
  ...
  return (
    <div className="no-scrollbar flex items-center gap-1 overflow-x-auto rounded-xl bg-white/15 p-1">
      {OPTIONS.map((opt) => (
        <button
          key={opt}
          ...
          disabled={pending}
          className={cn(
            "relative flex flex-1 items-center justify-center gap-1.5 whitespace-nowrap rounded-lg px-2 py-1.5 text-xs font-medium transition-colors disabled:opacity-80",
            range === opt ? "bg-white text-ink shadow-soft" : "text-white/85"
          )}
        >
          {pending && pendingOption === opt && <LoadingRing size={11} className="text-emerald-600" />}
          {RANGE_LABELS[opt]}
        </button>
      ))}
    </div>
