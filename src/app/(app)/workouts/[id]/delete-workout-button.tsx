...
export function DeleteWorkoutButton({ workoutId }: { workoutId: string }) {
  ...
  if (confirming) {
    return (
      <div className="flex items-center gap-2">
        <span className="text-sm text-black/50 dark:text-white/50">Delete?</span>
        <button
          disabled={pending}
          onClick={() => startTransition(() => deleteWorkoutAction(workoutId))}
  ...
  return (
    <button onClick={() => setConfirming(true)} className="flex items-center gap-1.5 text-sm font-medium text-red-500">
