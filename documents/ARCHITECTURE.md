...
| `blood_pressure_logs` | Blood Pressure log (`/blood-pressure`, v1.1.0) — `systolic`, `diastolic`, `measured_at`, `notes`. Deliberately its own table, same reasoning `weight_logs`' own comment already gives: a manually-logged metric with a real UI gets its own table, not folded into the import-only `health_metrics`. Same bug class #7 manual-SQL-Editor-step caveat applies. |
